...
SRecepteur = 0.4;  % Bruit sur la puissance du récepteur radio, en mW.
...
for iStep = 1:nStep
    ...
    % Estimation de la valheure
    zhat = Pwifi*exp(-X(1)/10);
    
    % ====== Calcul des matrices Jacobiennes ======
    F = [1 dT; 0 0];
    %G = [-(0.628319*sin(0.628319*dT))/(cos(0.628319*dT)^2+0.444444); 3/(X(1)^2+4)];
    G = [0; 3/(X(1)^2+4)];
    H = [-exp(-X(1)/10) 0];
