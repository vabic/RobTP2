...
d_init  = randi([3,12])     % Point de depart du robot inconnue. 3<d12
...
nParticules = C;
X(1,:) = 3 + 9*rand(1,nParticules); % position
X(2,:) = xVrai(2)*ones(1,nParticules); % vitesse
...
for iStep = 1:nStep
    ...
    legend(h,{'EKF','Position Exacte','Mesure h_z^{-1}'});
    message = sprintf('Le filtre à particule avec la position initial inconnue et C = %d',C);
    title(message)
    ylim([0 20]);
    drawnow();
end
